        ...
        for line in self.ach_lines:
            if line:
                if line[0] == self.FILE_HEADER:
                    self.ach_data["file_header"] = self.__parse_line(line, "FILE_HEADER_DEF")
                    break
        ...
        for line in self.ach_lines:
            if line:
                if line[0] == self.FILE_CONTROL:
                    self.ach_data["file_control"] = self.__parse_line(line, "FILE_CONTROL_DEF")
                    break
    ...
    def __parse_batches(self, batch_info):
        ...
        for batch in batch_info:
            self.ach_data["batches"].append(
                {
                    "batch_header": self.__parse_line(self.ach_lines[batch["batch_header_line"]], "BATCH_HEADER_DEF"),
                    "batch_control": self.__parse_line(
                        self.ach_lines[batch["batch_control_line"]], "BATCH_CONTROL_DEF"
                    ),
                    "entries": [],
                }
            )
                ...
                if self.ach_lines[line_num]:
                    ...
                    if self.ach_lines[line_num][0] == self.ENTRY_DETAIL:
                        self.ach_data["batches"][cur_batch]["entries"].append(
                            {
                                "entry_detail": self.__parse_line(self.ach_lines[line_num], "ENTRY_DETAIL_DEF"),
                                "addenda": [],
                            }
                        )
                    if self.ach_lines[line_num][0] == self.ADDENDA_RECORD:
                        self.ach_data["batches"][cur_batch]["entries"][cur_entry]["addenda"].append(
                            self.__parse_line(self.ach_lines[line_num], "ADDENDA_RECORD_DEF")
                        )
